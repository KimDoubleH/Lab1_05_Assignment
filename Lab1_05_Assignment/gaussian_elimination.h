...
void print(const double(&A)[MATRIX_SIZE][MATRIX_SIZE])	//print a matrix
{
	cout.setf(ios::fixed);
...
void print(const double(&b)[MATRIX_SIZE])	//print a vector
{
	cout.setf(ios::fixed);
	cout.setf(ios::showpoint);
	cout.precision(2);
	for (int i = 0; i < MATRIX_SIZE; i++) cout << b[i] << "\t";
	cout << endl;
}

void assign_row(double(&old_row)[MATRIX_SIZE], const double(&new_row)[MATRIX_SIZE]) {	//update a row in the matrix
	for (int i = 0; i < MATRIX_SIZE; i++)
	{
		old_row[i] = new_row[i];
	}
